...
@injectable()
export class TypescriptHero implements Activatable {
  constructor(
  ...
  public setup(): void {
    this.logger.debug(
      '[TypescriptHero] Setting up extension and activatables.',
    );
  ...
  public start(): void {
    this.logger.debug(
      '[TypescriptHero] Starting up extension and activatables.',
    );
    for (const activatable of this.activatables) {
      activatable.start();
    }
