...
export class Configuration {
  ...
  constructor(@inject(iocSymbols.extensionContext) context: ExtensionContext) {
    context.subscriptions.push(
      workspace.onDidChangeConfiguration(() =>
        this._configurationChanged.fire(),
      ),
    );
  ...
  public verbosity(): 'error' | 'warn' | 'info' | 'debug' {
    const verbosity = workspace
      .getConfiguration(sectionKey)
      .get<'error' | 'warn' | 'info' | 'debug'>('verbosity', 'warn');
