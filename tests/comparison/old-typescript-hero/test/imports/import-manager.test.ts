...
import { ImportManagerProvider, iocSymbols } from '../../../src/ioc-symbols';
...
describe('ImportManager', () => {
  describe('Typescript', () => {
    ...
    const provider: ImportManagerProvider = ioc.get<ImportManagerProvider>(
      iocSymbols.importManager,
    );
    ...
    describe('addDeclarationImport()', () => {
      ...
      it('should add a default import to the import index.', async () => {
        ...
        const declaration = new DefaultDeclaration(
          'defaultdeclaration',
          new File('path', rootPath, 0, 1),
        );
        ctrl.addDeclarationImport({ declaration, from: '../lib' });
    ...
    describe('commit()', () => {
      it('should not touch anything if nothing changed', async () => {
        ...
        expect(document.lineAt(0).text).to.equal(
          `import {Class1} from '../resourceIndex';`,
        );
      });
      ...
      it('should add a specifier with a default (first) and a normal (second) import to the doc', async () => {
        const ctrl = await provider(document);
        const def = new DefaultDeclaration(
          'defaultExport',
          new File(file.fsPath, rootPath, 1, 2),
        );
      ...
      it('should convert a default import when a normal specifier is added', async () => {
        ...
        await ctrl
          .addDeclarationImport({ declaration: def, from: '/server/indices' })
          .commit();

        expect(document.lineAt(0).text).to.matchSnapshot();

        await ctrl
          .addDeclarationImport({ declaration: dec, from: '/server/indices' })
          .commit();
      ...
      it('should render sorted imports when optimizing', async () => {
        await window.activeTextEditor!.edit(builder => {
          ...
          builder.insert(
            new Position(5, 0),
            'const foobar = new MultiExportClass();\n',
          );
        });
        const ctrl = await provider(document);
